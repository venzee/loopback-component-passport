...
PassportConfigurator.prototype.setupModels = function(options) {
  ...
  this.applicationModel = options.applicationModel || loopback.getModelByType(this.app.models.Application);
  this.oAuthTokenModel = options.oAuthTokenModel || loopback.getModelByType(this.app.models.oAuthTokenModel);
  ...
  // loopback oauth2 models
  if (this.oAuthTokenModel){
    this.oAuthTokenModel.belongsTo(this.userModel,
      {as: 'user', foreignKey: 'userId'});
  }
  if (this.oAuthTokenModelv && this.applicationModel ){
    this.oAuthTokenModel.belongsTo(this.applicationModel,
...
PassportConfigurator.prototype.configureProvider = function(name, options) {
  ...
  var appendAccessToken = function(url, accessToken) {
    ...
    return url + '?access-token=' + accessToken.id + '&refresh-token=' + accessToken.refreshToken + '&user-id=' + accessToken.userId;
  };
