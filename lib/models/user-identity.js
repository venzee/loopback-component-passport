...
function UserIdentity(UserIdentity) {
  ...
  */
  function createAccessToken(user, ttl, options, cb) {
    ...
    var token = generateToken({
      ...
      client: options ? options.appClientId: null,
      user: user,
      scope: null
    });
    ...
    if (user.tokens){
      user.tokens.create({
        id: token.id,
        appId: options ? options.appClientId: null,
        refreshToken: refreshToken.id,
        issuedAt: new Date(),
        userId: user.id,
        expiresIn: Math.min(ttl || user.constructor.settings.ttl,
          user.constructor.settings.maxTTL),
      }, function(err, model){
        if (err){
          cb(err);
        } else {
          model.created = model.issuedAt;
          model.ttl = model.expiresIn;
          cb(null, model);
        }
      });
        ...
        return identity.updateAttributes({profile: profile,
          ...
          return identity.user(function(err, user) {
            ...
            if (!err && user && autoLogin) {
              return (options.createAccessToken || createAccessToken)(user, null, options, function(err, token) {
